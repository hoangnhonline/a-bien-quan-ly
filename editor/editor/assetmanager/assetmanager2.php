<?php
session_start();
if ($_SESSION['is_admin_xem']  <> true)
{
	header ("location:/index.php");
}
$realpath = realpath('../../../')."/files_upload/chapters";
$realpath = str_replace("\\","/",$realpath);
$bReturnAbsolute=false;

$sBaseVirtual0="/files_upload/chapters";

$sBase0=$realpath; //The real path 

$sName0="/files_upload/chapters";
...
$AllowedTypes = "pdf|doc|xls|psd|gif|jpg|png|bmp|flv|wmv|avi|mpg|mp4";
//$AllowedTypes = "*";
...
function writeFileSelections()
  ...
  echo "<div style='overflow:auto;height:350px;width:100%;margin-top:3px;margin-bottom:2px;'>";
    ...
    if($sItem=="."||$sItem=="..")
      ...
      if(!$fIsDirectory)
        ...
        if($ffilter=="flash")
          ...
          {
          if ($sExt=="mp4" || $sExt=="avi" || $sExt=="wmv" || $sExt=="mpg" || $sExt=="mpeg" || $sExt=="wav" || $sExt=="wma" || $sExt=="mid" || $sExt=="mp3") $bDisplay=true;
          }
...
function getAction()
  ...
  if(sQueryString.indexOf("=")==-1)
    {//no querystring
    sAction="assetmanager2.php?ffilter="+document.getElementById("selFilter").value;
    }
  else
    {
    sAction="assetmanager2.php?"+sQueryString+"&ffilter="+document.getElementById("selFilter").value
    }
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  return sAction;
  }
...
function selectFile(index)
  ...
  parent.mfileclick(document.getElementById("inpSource").value, '<?echo($_GET['target']);?>');
...
function doOk()
  ...
  if(navigator.appName.indexOf('Microsoft')!=-1)
    ...
    window.opener.fileclick(document.getElementById("inpSource").value);
  bOk=true;
  self.close();
  }
function doUnload()
  ...
  if(navigator.appName.indexOf('Microsoft')!=-1)
    ...
    if(!bOk)window.opener.fileclick("");
  }
</script>
</head>
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
    ...
    <td valign=top align="center">
        ...
        <tr>
        <td>
          <div id="idPreview" style="text-align:center;overflow:auto;width:297;height:345;border:#d7d7d7 5px solid;border-bottom:#d7d7d7 3px solid;background:#ffffff;margin-right:2;"></div>
          <div align=center><input type="hidden" id="inpSource" name="inpSource" style="border:#cfcfcf 1px solid;width:295" class="inpTxt"></div>
        </td>
...
<td class="dialogFooter" style="height:40px;padding-right:15px;" align=right valign=middle>
  ...
  <tr>
  <td>
  <input style="display:none;" name="btnOk" id="btnOk" type="button" value=" ok " onclick="doOk()" class="inpBtn" onmouseover="this.className='inpBtnOver';" onmouseout="this.className='inpBtnOut'">
  </td>
  </tr>
  </table>
</td>
