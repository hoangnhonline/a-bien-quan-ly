...
if ($act=="addPro" OR $act=="editPro"){
	...
	while($sql_query_rows = mysql_fetch_array($sql_query))
	{
		...
		$template->assign_vars(array(
			...
			'txt_user_mobile1'	=>  ($pro_admin_id==$_SESSION['login_id'] OR $_SESSION['is_admin'] OR (int)$sql_query_rows['wait_user']==1 OR checkExpired($sql_query_rows['update_time'])) ? Make_field_values1("mobile1", $table_u, $db," where id='".$user_id."'"):"",
			'txt_user_mobile2'	=>  ($pro_admin_id == $_SESSION['login_id'] OR $_SESSION['is_admin'] OR (int)$sql_query_rows['wait_user'] == 1 OR checkExpired($sql_query_rows['update_time']))  ? Make_field_values1("mobile2", $table_u, $db," where id='".$user_id."'"):"",
